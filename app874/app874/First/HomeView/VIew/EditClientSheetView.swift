                ...
                VStack {
                    ...
                    Button {
                        if !name.isEmpty && !proceduresPerformed.isEmpty && !proceduresTotal.isEmpty && !income.isEmpty {
                            if let image = selectedImage {
                                client = Client(imageData: image.jpegData(compressionQuality: 1.0) ,name: name, proceduresPerformed: Int(proceduresPerformed) ?? 0, totalProcedures: Int(proceduresTotal) ?? 0, income: income)
                                viewModel.updateClient(for: client)
                            } else {
                                client = Client(name: name, proceduresPerformed: Int(proceduresPerformed) ?? 0, totalProcedures: Int(proceduresTotal) ?? 0, income: income)
                                viewModel.updateClient(for: client)
                            }
                            isPresented = false
...
#Preview {
    EditClientSheetView(viewModel: HomeViewModel(), isPresented: .constant(true), client: .constant(Client(name: "DIAS ATUDINOV", proceduresPerformed: 100, totalProcedures: 200, income: "$1200")))
}
